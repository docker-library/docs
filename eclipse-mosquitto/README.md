...
```console
$ docker run -it -p 1883:1883 -v "$PWD/mosquitto/config:/mosquitto/config" amd64/eclipse-mosquitto
```
...
```console
$ docker run -it -p 1883:1883 -v "$PWD/mosquitto/config:/mosquitto/config" -v /mosquitto/data -v /mosquitto/log amd64/eclipse-mosquitto
```

or:

```console
$ docker run -it -p 1883:1883 -v "$PWD/mosquitto/config:/mosquitto/config" -v "$PWD/mosquitto/data:/mosquitto/data" -v "$PWD/mosquitto/log:/mosquitto/log" amd64/eclipse-mosquitto
```
...
```console
$ docker run -it -p 1883:1883 -p 8080:8080 -v "$PWD/mosquitto/config:/mosquitto/config" amd64/eclipse-mosquitto
```

# Image Variants

The `amd64/eclipse-mosquitto` images come in many flavors, each designed for a specific use case.

## `amd64/eclipse-mosquitto:<version>`

This is the defacto image. If you are unsure about what your needs are, you probably want to use this one. It is designed to be used both as a throw away container (mount your source code and start the container to start your app), as well as the base to build other images off of.

## `amd64/eclipse-mosquitto:<version>-alpine`
