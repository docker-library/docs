...
```console
$ docker run -it -p 1883:1883 -v "$PWD/mosquitto/config:/mosquitto/config" winamd64/eclipse-mosquitto
```
...
```console
$ docker run -it -p 1883:1883 -v "$PWD/mosquitto/config:/mosquitto/config" -v /mosquitto/data -v /mosquitto/log winamd64/eclipse-mosquitto
```

or:

```console
$ docker run -it -p 1883:1883 -v "$PWD/mosquitto/config:/mosquitto/config" -v "$PWD/mosquitto/data:/mosquitto/data" -v "$PWD/mosquitto/log:/mosquitto/log" winamd64/eclipse-mosquitto
```
...
```console
$ docker run -it -p 1883:1883 -p 8080:8080 -v "$PWD/mosquitto/config:/mosquitto/config" winamd64/eclipse-mosquitto
```

# Image tags

The supported-tags list at the top of this page is authoritative. Official `winamd64/eclipse-mosquitto` images are Alpine-based. Tag naming does **not** follow a generic "plain version" vs "version-alpine" split:
