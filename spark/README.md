...
**Note:** this is the "per-architecture" repository for the `ppc64le` builds of [the `spark` official image](https://hub.docker.com/_/spark) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![ppc64le/spark build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/spark.svg?label=ppc64le/spark%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/spark/)
...
```console
docker run -it ppc64le/spark /opt/spark/bin/spark-shell
```
...
```console
docker run -it ppc64le/spark:python3 /opt/spark/bin/pyspark
```
...
```console
docker run -it ppc64le/spark:r /opt/spark/bin/sparkR
```
