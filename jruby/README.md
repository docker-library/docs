...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app arm32v7/jruby:10 bundle install --system
```
...
```console
$ docker run -it --rm --name my-running-script -v "$PWD":/usr/src/myapp -w /usr/src/myapp arm32v7/jruby:10 jruby your-daemon-or-script.rb
```
