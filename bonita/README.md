...
```bash
docker run --name=bonita -h <hostname> --env-file=env.txt -d -p 8080:8080 mips64le/bonita
```

## Start Bonita with custom security credentials

```bash
docker run --name=bonita -h <hostname> -e "BONITA_RUNTIME_ADMIN_USERNAME=tech_user" -e "BONITA_RUNTIME_ADMIN_PASSWORD=secret" -e "PLATFORM_LOGIN=pfadmin" -e "PLATFORM_PASSWORD=pfsecret" -d -p 8080:8080 mips64le/bonita
```
...
```console
$ docker run  -e HTTP_API=true -e HTTP_API_PASSWORD="My-Cust0m_S3cR3T" --name bonita -d -p 8080:8080 mips64le/bonita
```
...
-	Retrieve the last update tool archive from https://www.bonitasoft.com/downloads
	...
	```console
	unzip bonita-update-tool-3.10.0.zip
	```

-	Configure the update tool

	```console
	$ cd bonita-update-tool-3.10.0
	```
...
-	Launch the new container pointing towards the copy of the database.
	...
	```console
	$ docker run --name=bonita --link mydbpostgres:postgres -e "DB_NAME=newbonitadb" -e "DB_USER=newbonitauser" -e "DB_PASS=newbonitapass" -d -p 8081:8080 mips64le/bonita:2026.2-u0
	```
