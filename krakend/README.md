...
**Note:** this is the "per-architecture" repository for the `ppc64le` builds of [the `krakend` official image](https://hub.docker.com/_/krakend) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![ppc64le/krakend build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/krakend.svg?label=ppc64le/krakend%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/ppc64le/job/krakend/)
...
```bash
docker run -d -p 8080:8080 -v "$PWD:/etc/krakend/" ppc64le/krakend
...
```bash
docker run -p 8080:8080 -v "${PWD}:/etc/krakend/" ppc64le/krakend run -d -c /etc/krakend/krakend.json
```
...
```bash
docker run -it -v $PWD:/etc/krakend/ ppc64le/krakend check --config krakend.json
```

#### Show the help:

```bash
docker run --rm -it ppc64le/krakend help
```

### Building your custom KrakenD image

Most production deployments will not want to rely on mounting a volume for the container but to use their image based on `ppc64le/krakend`:

Your `Dockerfile` could look like this:

```Dockerfile
FROM ppc64le/krakend:<version>
...
```Dockerfile
FROM ppc64le/krakend:<version> as builder
...
```bash
docker run --rm -it ppc64le/krakend help
docker run --rm -it ppc64le/krakend krakend help
```
