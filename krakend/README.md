...
**Note:** this is the "per-architecture" repository for the `s390x` builds of [the `krakend` official image](https://hub.docker.com/_/krakend) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![s390x/krakend build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/s390x/job/krakend.svg?label=s390x/krakend%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/s390x/job/krakend/)
...
```bash
docker run -d -p 8080:8080 -v "$PWD:/etc/krakend/" s390x/krakend
...
```bash
docker run -p 8080:8080 -v "${PWD}:/etc/krakend/" s390x/krakend run -d -c /etc/krakend/krakend.json
```
...
```bash
docker run -it -v $PWD:/etc/krakend/ s390x/krakend check --config krakend.json
```

#### Show the help:

```bash
docker run --rm -it s390x/krakend help
```

### Building your custom KrakenD image

Most production deployments will not want to rely on mounting a volume for the container but to use their image based on `s390x/krakend`:

Your `Dockerfile` could look like this:

```Dockerfile
FROM s390x/krakend:<version>
...
```Dockerfile
FROM s390x/krakend:<version> as builder
...
```bash
docker run --rm -it s390x/krakend help
docker run --rm -it s390x/krakend krakend help
```
