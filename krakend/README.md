...
**Note:** this is the "per-architecture" repository for the `arm32v6` builds of [the `krakend` official image](https://hub.docker.com/_/krakend) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![arm32v6/krakend build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/krakend.svg?label=arm32v6/krakend%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v6/job/krakend/)
...
```bash
docker run -d -p 8080:8080 -v "$PWD:/etc/krakend/" arm32v6/krakend
...
```bash
docker run -p 8080:8080 -v "${PWD}:/etc/krakend/" arm32v6/krakend run -d -c /etc/krakend/krakend.json
```
...
```bash
docker run -it -v $PWD:/etc/krakend/ arm32v6/krakend check --config krakend.json
```

#### Show the help:

```bash
docker run --rm -it arm32v6/krakend help
```

### Building your custom KrakenD image

Most production deployments will not want to rely on mounting a volume for the container but to use their image based on `arm32v6/krakend`:

Your `Dockerfile` could look like this:

```Dockerfile
FROM arm32v6/krakend:<version>
...
```Dockerfile
FROM arm32v6/krakend:<version> as builder
...
```bash
docker run --rm -it arm32v6/krakend help
docker run --rm -it arm32v6/krakend krakend help
```
