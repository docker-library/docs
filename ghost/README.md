...
The `next` tags (`i386/ghost:next`, `i386/ghost:6-next`, `i386/ghost:next-alpine`, plus fully qualified variants such as `i386/ghost:6.61.0-next-alpine3.23`) are a preview of the image that will become the standard `i386/ghost` image in **Ghost 7.0**. They track the same stable upstream Ghost releases as the default tags; what differs is how the image is built and laid out, not which version of Ghost is inside. The `next` tags never carry `latest`.

When Ghost 7.0 ships, this layout becomes the plain `i386/ghost:<version>` image and the `next` tags go away, so trying `next` now is the way to find breakage in your setup ahead of that switch.
