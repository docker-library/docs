...
```console
$ docker run -d --name some-ghost riscv64/ghost
```
...
```console
$ docker run -d --name some-ghost -e url=http://localhost:3001 -p 3001:2368 riscv64/ghost
```
...
```console
$ docker run -d --name some-ghost -p 3001:2368 -v /path/to/ghost/blog:/var/lib/ghost/content riscv64/ghost:alpine
```
...
```console
$ docker run -d --name some-ghost -v some-ghost-data:/var/lib/ghost/content riscv64/ghost
```
...
```console
$ docker run -d --name some-ghost -e url=http://some-ghost.example.com riscv64/ghost
```
