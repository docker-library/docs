...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 mips64le/eclipse-temurin:25
```
...
```dockerfile
FROM mips64le/eclipse-temurin:25
RUN mkdir /opt/app
...
COPY --from=mips64le/eclipse-temurin:25 $JAVA_HOME $JAVA_HOME
...
FROM mips64le/eclipse-temurin:25 as jre-build
