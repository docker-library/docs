...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 riscv64/eclipse-temurin:25
```
...
```dockerfile
FROM riscv64/eclipse-temurin:25
RUN mkdir /opt/app
...
COPY --from=riscv64/eclipse-temurin:25 $JAVA_HOME $JAVA_HOME
...
FROM riscv64/eclipse-temurin:25 as jre-build
