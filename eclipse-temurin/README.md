...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 mips64le/eclipse-temurin:21
```
...
```dockerfile
FROM mips64le/eclipse-temurin:21
RUN mkdir /opt/app
...
COPY --from=mips64le/eclipse-temurin:21 $JAVA_HOME $JAVA_HOME
...
FROM mips64le/eclipse-temurin:21 as jre-build
...
```dockerfile
FROM mips64le/eclipse-temurin:21.0.2_13-jdk
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
