...
```console
$ docker run -v $(pwd)/certs:/certificates/ -e USE_SYSTEM_CA_CERTS=1 arm32v6/eclipse-temurin:21
```
...
```dockerfile
FROM arm32v6/eclipse-temurin:21
RUN mkdir /opt/app
...
COPY --from=arm32v6/eclipse-temurin:21 $JAVA_HOME $JAVA_HOME
...
FROM arm32v6/eclipse-temurin:21 as jre-build
...
```dockerfile
FROM arm32v6/eclipse-temurin:21.0.2_13-jdk
CMD ["java", "-jar", "/opt/app/japp.jar"]
```
