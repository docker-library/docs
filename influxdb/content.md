...
The InfluxDB image contains some extra functionality to automatically bootstrap the system. This functionality is enabled by setting the `INFLUXDB_INIT_MODE` environment variable to the value `setup` when running the container. Additional environment variables are used to configure the setup logic:

-	`INFLUXDB_INIT_USERNAME`: The username to set for the system's initial super-user (**Required**).
-	`INFLUXDB_INIT_PASSWORD`: The password to set for the system's inital super-user (**Required**).
-	`INFLUXDB_INIT_ORG`: The name to set for the system's initial organization (**Required**).
-	`INFLUXDB_INIT_BUCKET`: The name to set for the system's initial bucket (**Required**).
-	`INFLUXDB_INIT_RETENTION`: The duration the system's initial bucket should retain data. If not set, the initial bucket will retain data forever.
-	`INFLUXDB_INIT_ADMIN_TOKEN`: The authentication token to associate with the system's initial super-user. If not set, a token will be auto-generated by the system.
...
$ docker run -d -p 8086:8086 \
      ...
      -e INFLUXDB_INIT_USERNAME=my-user \
      -e INFLUXDB_INIT_PASSWORD=my-password \
      -e INFLUXDB_INIT_ORG=my-org \
      -e INFLUXDB_INIT_BUCKET=my-bucket \
      %%IMAGE%%
```
...
$ docker run -d -p 8086:8086 \
      ...
      -e INFLUXDB_INIT_ORG=my-org \
      -e INFLUXDB_INIT_BUCKET=my-bucket \
      -e INFLUXDB_INIT_RETENTION=1w \
      -e INFLUXDB_INIT_ADMIN_TOKEN=my-super-secret-auth-token \
      %%IMAGE%%
```
...
Finally, the `INFLUXDB_INIT_MODE` environment variable must be set to `upgrade`.
...
$ docker run -p 8086:8086 \
      ...
      -e INFLUXDB_INIT_BUCKET=my-bucket \
      -e INFLUXDB_INIT_UPGRADE_V1_CONFIG=/root/influxdb/influxdb.conf \
      -e INFLUXDB_CONFIG_PATH=/root/influxdb2/config.toml \
      -e INFLUXDB_BOLT_PATH=/root/influxdb2/influxdb.bolt \
      -e INFLUXDB_ENGINE_PATH=/root/influxdb2/engine \
      %%IMAGE%%
```
