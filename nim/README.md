...
**Note:** this is the "per-architecture" repository for the `arm64v8` builds of [the `nim` official image](https://hub.docker.com/_/nim) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
-	[`2.2.12`, `2.2`, `2`, `latest`](https://github.com/nim-lang/docker-images/blob/6f1fe6f6b7069e29ee5fd1b3fe1ddb5c613603c5/dockerfiles/2.2.12/Dockerfile)

-	[`2.0.14`, `2.0`](https://github.com/nim-lang/docker-images/blob/6f1fe6f6b7069e29ee5fd1b3fe1ddb5c613603c5/dockerfiles/2.0.14/Dockerfile)
...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app arm64v8/nim nim c -r main.nim
```
...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app arm64v8/nim nim js main.nim
```

To compile and run, use a multi-stage Dockerfile with Node.js:

```dockerfile
FROM arm64v8/nim AS builder
COPY . .
...
```console
$ docker run --rm -v "$PWD":/usr/src/app -w /usr/src/app arm64v8/nim nimble install -y
```
