...
The i386/mariadb has a number of tags, and of note is `latest`, as the latest stable version, and `lts`, as the last long term support release.
...
### Start a `i386/mariadb` server instance with user, password and database
...
### Start a `i386/mariadb` server instance in a network
...
The following command starts another `i386/mariadb` container instance and runs the `mariadb` command line client against your original `i386/mariadb` container, allowing you to execute SQL statements against your database instance:
...
```console
$ docker run -it --rm i386/mariadb mariadb --host <server container IP> --user example-user --password --database test
```
...
The `docker exec` command allows you to run commands inside the running container. The following command line will give you a bash shell inside your `i386/mariadb` container:
...
The `i386/mariadb` image configuration contains the Ubuntu MariaDB variables with two custom changes for the container:
...
These disable the authentication of `user@hostname` users. To re-enable the `skip-name-resolve` use `disable-skip-name-resolve` as variable or argument. When enabled, the `host-cache-size` should be sufficient for the number of containers connecting to the `i386/mariadb`.

To view the resulting configuration of your `i386/mariadb` container:

```console
$ docker run --name some-mariadb -v /my/custom:/etc/mysql/conf.d --rm i386/mariadb:latest my_print_defaults --mariadbd
```
...
```console
$ docker run --rm i386/mariadb:latest --verbose --help
```

## Environment Variables

When you start the `i386/mariadb` image, you can adjust the initialization of the MariaDB instance by passing one or more environment variables on the `docker run` command line. Do note that all of the variables, except `MARIADB_AUTO_UPGRADE`, will have no effect if you start the container with a data directory that already contains a database. I.e. any pre-existing database will always be left untouched on container startup.
...
Example compose files using this `i386/mariadb` are located in https://github.com/MariaDB/mariadb-docker in the `/examples` folder.
