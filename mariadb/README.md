...
The arm32v6/mariadb has a number of tags, and of note is `latest`, as the latest stable version, and `lts`, as the last long term support release.
...
```console
$ docker run --name some-mariadb -p 3306:3306 arm32v6/mariadb:latest
```
...
```console
$ docker run --detach --name some-mariadb --env MARIADB_ROOT_PASSWORD=my-secret-pw arm32v6/mariadb:latest
```

or:

```console
$ docker run --detach --name some-mariadb --env MARIADB_ALLOW_EMPTY_ROOT_PASSWORD=1 arm32v6/mariadb:latest
```

or:

```console
$ docker run --detach --name some-mariadb --env MARIADB_RANDOM_ROOT_PASSWORD=1 arm32v6/mariadb:latest
```
...
### Start a `arm32v6/mariadb` server instance with user, password and database

Starting a MariaDB instance with a user, password, and a database:

```console
$ docker run --detach --name some-mariadb --env MARIADB_USER=example-user --env MARIADB_PASSWORD=my_cool_secret --env MARIADB_DATABASE=exmple-database --env MARIADB_ROOT_PASSWORD=my-secret-pw arm32v6/mariadb:latest
```

### Start a `arm32v6/mariadb` server instance in a network
...
The following command starts another `arm32v6/mariadb` container instance and runs the `mariadb` command line client against your original `arm32v6/mariadb` container, allowing you to execute SQL statements against your database instance:

```console
$ docker run -it --network some-network --rm arm32v6/mariadb mariadb -h some-mariadb -u example-user
```
...
```console
$ docker run -it --rm arm32v6/mariadb mariadb --host <server container IP> --user example-user --password --database test
```
...
The `docker exec` command allows you to run commands inside the running container. The following command line will give you a bash shell inside your `arm32v6/mariadb` container:
...
The `arm32v6/mariadb` image configuration contains the Ubuntu MariaDB variables with two custom changes for the container:
...
These disable the authentication of `user@hostname` users. To re-enable the `skip-name-resolve` use `disable-skip-name-resolve` as variable or argument. When enabled, the `host-cache-size` should be sufficient for the number of containers connecting to the `arm32v6/mariadb`.

To view the resulting configuration of your `arm32v6/mariadb` container:

```console
$ docker run --name some-mariadb -v /my/custom:/etc/mysql/conf.d --rm arm32v6/mariadb:latest my_print_defaults --mariadbd
```
...
```console
$ docker run --rm arm32v6/mariadb:latest --verbose --help
```

## Environment Variables

When you start the `arm32v6/mariadb` image, you can adjust the initialization of the MariaDB instance by passing one or more environment variables on the `docker run` command line. Do note that all of the variables, except `MARIADB_AUTO_UPGRADE`, will have no effect if you start the container with a data directory that already contains a database. I.e. any pre-existing database will always be left untouched on container startup.
...
```console
$ docker run --name some-mysql -e MARIADB_ROOT_PASSWORD_FILE=/run/secrets/mariadb-root -d arm32v6/mariadb:latest
```
...
2.	Start your `arm32v6/mariadb` container like this:
	...
	```console
	$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql:Z -e MARIADB_ROOT_PASSWORD=my-secret-pw -d arm32v6/mariadb:latest
	```
...
Example compose files using this `arm32v6/mariadb` are located in https://github.com/MariaDB/mariadb-docker in the `/examples` folder.
