...
The winamd64/mariadb has a number of tags, and of note is `latest`, as the latest stable version, and `lts`, as the last long term support release.
...
```console
$ docker run --name some-mariadb -p 3306:3306 winamd64/mariadb:latest
```
...
```console
$ docker run --detach --name some-mariadb --env MARIADB_ROOT_PASSWORD=my-secret-pw winamd64/mariadb:latest
```

or:

```console
$ docker run --detach --name some-mariadb --env MARIADB_ALLOW_EMPTY_ROOT_PASSWORD=1 winamd64/mariadb:latest
```

or:

```console
$ docker run --detach --name some-mariadb --env MARIADB_RANDOM_ROOT_PASSWORD=1 winamd64/mariadb:latest
```
...
### Start a `winamd64/mariadb` server instance with user, password and database

Starting a MariaDB instance with a user, password, and a database:

```console
$ docker run --detach --name some-mariadb --env MARIADB_USER=example-user --env MARIADB_PASSWORD=my_cool_secret --env MARIADB_DATABASE=exmple-database --env MARIADB_ROOT_PASSWORD=my-secret-pw winamd64/mariadb:latest
```

### Start a `winamd64/mariadb` server instance in a network

As applications talk to MariaDB, MariaDB needs to start in the same network as the application:

```console
$ docker network create some-network 
$ docker run --detach --network some-network --name some-mariadb --env MARIADB_USER=example-user --env MARIADB_PASSWORD=my_cool_secret --env MARIADB_ROOT_PASSWORD=my-secret-pw winamd64/mariadb:latest
...
The following command starts another `winamd64/mariadb` container instance and runs the `mariadb` command line client against your original `winamd64/mariadb` container, allowing you to execute SQL statements against your database instance:

```console
$ docker run -it --network some-network --rm winamd64/mariadb mariadb -h some-mariadb -u example-user
```
...
```console
$ docker run -it --rm winamd64/mariadb mariadb --host <server container IP> --user example-user --password --database test
```
...
The `docker exec` command allows you to run commands inside the running container. The following command line will give you a bash shell inside your `winamd64/mariadb` container:
...
The `winamd64/mariadb` image configuration contains the Ubuntu MariaDB variables with two custom changes for the container:
...
These disable the authentication of `user@hostname` users. To re-enable the `skip-name-resolve` use `disable-skip-name-resolve` as variable or argument. When enabled, the `host-cache-size` should be sufficient for the number of containers connecting to the `winamd64/mariadb`.

To view the resulting configuration of your `winamd64/mariadb` container:

```console
$ docker run --name some-mariadb -v /my/custom:/etc/mysql/conf.d --rm winamd64/mariadb:latest my_print_defaults --mysqld
```
...
```console
$ docker run --name some-mariadb -e MARIADB_ROOT_PASSWORD=my-secret-pw -d winamd64/mariadb:latest --port 3808
```

If you would like to see a complete list of available options, just run:

```console
$ docker run -it --rm winamd64/mariadb:latest --verbose --help
```

## Environment Variables

When you start the `winamd64/mariadb` image, you can adjust the initialization of the MariaDB instance by passing one or more environment variables on the `docker run` command line. Do note that all of the variables, except `MARIADB_AUTO_UPGRADE`, will have no effect if you start the container with a data directory that already contains a database. I.e. any pre-existing database will always be left untouched on container startup.
...
```console
$ docker run --name some-mysql -e MARIADB_ROOT_PASSWORD_FILE=/run/secrets/mariadb-root -d winamd64/mariadb:latest
```
...
2.	Start your `winamd64/mariadb` container like this:
	...
	```console
	$ docker run --name some-mariadb -v /my/own/datadir:/var/lib/mysql:Z -e MARIADB_ROOT_PASSWORD=my-secret-pw -d winamd64/mariadb:latest
	```
...
Example compose files using this `winamd64/mariadb` are located in https://github.com/MariaDB/mariadb-docker in the `/examples` folder.
