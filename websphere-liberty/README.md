...
**Note:** this is the "per-architecture" repository for the `riscv64` builds of [the `websphere-liberty` official image](https://hub.docker.com/_/websphere-liberty) -- for more information, see ["Architectures other than amd64?" in the official images documentation](https://github.com/docker-library/official-images#architectures-other-than-amd64) and ["An image's source changed in Git, now what?" in the official images FAQ](https://github.com/docker-library/faq#an-images-source-changed-in-git-now-what).
...
[![riscv64/websphere-liberty build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/websphere-liberty.svg?label=riscv64/websphere-liberty%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/riscv64/job/websphere-liberty/)
...
```dockerfile
FROM riscv64/websphere-liberty:kernel
...
$ docker run -d -p 80:9080 -p 443:9443 \
	    ...
	    riscv64/websphere-liberty:webProfile8
```
...
2.	The `full` images provide the library cache directory, `lib.index.cache`, which contains an indexed library cache created by the `springBootUtility` command. Use `lib.index.cache` to provide the library cache for a thin application.
	...
	```dockerfile
	FROM riscv64/websphere-liberty:kernel as staging
...
docker run -e LICENSE=accept -v /opt/ibm/wlp/output/.classCache \
    --name classcache riscv64/websphere-liberty true
```
...
docker run -d -p 80:9080 -p 443:9443 \
    ...
    riscv64/websphere-liberty:javaee8
```

# Changing locale

The base Ubuntu image does not include additional language packs. To use an alternative locale, build your own image that installs the required language pack and then sets the `LANG` environment variable. For example, the following Dockerfile starts with the `riscv64/websphere-liberty:full` image, installs the Portuguese language pack, and sets Brazilian Portuguese as the default locale:

```dockerfile
FROM riscv64/websphere-liberty:full
RUN apt-get update \
