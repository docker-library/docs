...
```console
docker run -d -p 8529:8529 -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name arangodb-instance riscv64/arangodb
```
...
```console
docker run -d -p 8529:8529 -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name arangodb-instance --platform linux/arm64/v8 riscv64/arangodb
```

This creates and launches the riscv64/arangodb Docker instance as a background process. The Identifier of the process is printed. By default, ArangoDB listens on port `8529` for requests.
...
-	From v3.12.2 onward: Append the `--icu-language` startup option to the end of the Docker command, like `docker run ... riscv64/arangodb --icu-language sv` for a Swedish locale.
...
```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name my-app --link arangodb-instance:db-link riscv64/arangodb
```
...
```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -p 8529:8529 -d riscv64/arangodb
```
...
```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 riscv64/arangodb --help
```
...
```console
docker run -it riscv64/arangodb arangosh --server.database myDB ...
```
...
See `docker inspect --format "{{ .Config.Volumes }}" riscv64/arangodb` for all volumes.
...
```console
docker create --name arangodb-persist riscv64/arangodb true
```

And use this data container in your ArangoDB container.

```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --volumes-from arangodb-persist -p 8529:8529 riscv64/arangodb
```
