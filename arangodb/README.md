...
```console
docker run -d -p 8529:8529 -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name arangodb-instance ppc64le/arangodb
```
...
```console
docker run -d -p 8529:8529 -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name arangodb-instance --platform linux/arm64/v8 ppc64le/arangodb
```

This creates and launches the ppc64le/arangodb Docker instance as a background process. The Identifier of the process is printed. By default, ArangoDB listens on port `8529` for requests.
...
-	From v3.12.2 onward: Append the `--icu-language` startup option to the end of the Docker command, like `docker run ... ppc64le/arangodb --icu-language sv` for a Swedish locale.
...
```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --name my-app --link arangodb-instance:db-link ppc64le/arangodb
```
...
```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 -p 8529:8529 -d ppc64le/arangodb
```
...
```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 ppc64le/arangodb --help
```
...
```console
docker run -it ppc64le/arangodb arangosh --server.database myDB ...
```
...
See `docker inspect --format "{{ .Config.Volumes }}" ppc64le/arangodb` for all volumes.
...
```console
docker create --name arangodb-persist ppc64le/arangodb true
```

And use this data container in your ArangoDB container.

```console
docker run -e ARANGO_RANDOM_ROOT_PASSWORD=1 --volumes-from arangodb-persist -p 8529:8529 ppc64le/arangodb
```
