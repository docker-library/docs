...
```bash
docker run -it --rm --network=container:some-clickhouse-server --entrypoint clickhouse-client winamd64/clickhouse
# OR
...
```bash
docker run -d -p 18123:8123 -p 19000:9000 -e CLICKHOUSE_PASSWORD=changeme --name some-clickhouse-server --ulimit nofile=262144:262144 winamd64/clickhouse
...
```bash
docker run --rm -e CLICKHOUSE_SKIP_USER_SETUP=1 -p 9000:9000/tcp winamd64/clickhouse
```
