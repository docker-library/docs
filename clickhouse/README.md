...
[![arm32v5/clickhouse build status badge](https://img.shields.io/jenkins/s/https/doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/clickhouse.svg?label=arm32v5/clickhouse%20%20build%20job)](https://doi-janky.infosiftr.net/job/multiarch/job/arm32v5/job/clickhouse/)
...
```bash
docker run -it --rm --link some-clickhouse-server:clickhouse-server --entrypoint clickhouse-client arm32v5/clickhouse --host clickhouse-server
# OR
...
```bash
docker run -d -p 18123:8123 -p19000:9000 --name some-clickhouse-server --ulimit nofile=262144:262144 arm32v5/clickhouse
