...
```console
$ docker run -p 8080:80 --ulimit memlock=-1:-1 --tmpfs /var/lib/varnish/varnishd:exec riscv64/varnish
```
...
```console
$ docker run --rm riscv64/varnish varnishreload -h
```
...
Additionally, you can add arguments to `docker run` after `riscv64/varnish`, if the first argument starts with a `-`, the whole list will be appended to the [default command](https://github.com/varnish/docker-varnish/blob/master/fresh/debian/scripts/docker-varnish-entrypoint):
...
$ docker run \
	...
	--tmpfs /var/lib/varnish/varnishd:exec \
	-p 8080:80 \
	riscv64/varnish -p default_keep=300
```
