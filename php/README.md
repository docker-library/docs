...
For production deployments, this image ships with defaults that are quite different from standard Linux distros (Ubuntu).
This improves configurability but may cause unexpected performance loss.
A good example of that is opcache, recommend in in certain [PHP frameworks](https://symfony.com/doc/current/performance.html#performance-use-preloading).
Opcache is not by default enabled in this image.
...
A reasonable defaults `opcache.ini`  is show below
...
opcache.blacklist_filename=/etc/php-7.3.d/opcache*.blacklist
